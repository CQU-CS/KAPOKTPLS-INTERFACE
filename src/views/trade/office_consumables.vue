  ...
  <div>
      ...
      <el-card class="box-card" shadow="always" :body-style="{padding: '0px'}">
        <div style="margin-left: 15px;margin-right: 15px;">
          <el-table v-loading="loading" element-loading-text="拼命加载中"
            element-loading-background="rgba(255, 255, 255, 0.5)" :height="fullHeight" :data="officeMaterialPurchaseList" stripe
            style="width: 100%;" :row-style="{height:'40px'}" :cell-style="{padding:'0px'}">
            <el-table-column width="80px;" align="center" prop="officeMaterialPurchaseId" label="编号" sortable>
            </el-table-column>
            <el-table-column align="center" show-overflow-tooltip prop="officeMaterialName" label="办公耗材">
            </el-table-column>
            <el-table-column align="center" width="200px;" prop="companyName" label="公司">
            </el-table-column>
            <el-table-column align="center" width="150px;" show-overflow-tooltip prop="officeMaterialPurchasePrice" label="价格" sortable>
            </el-table-column>
            <el-table-column align="center" width="150px;" prop="officeMaterialPurchaseDate" label="时间">
            </el-table-column>
    ...
    <el-dialog :title="dialogName" :visible.sync="dialogFormVisible" center width="40%">
      <el-form :model="form" :rules="rules" ref="form" style="text-align: center;">
        <el-form-item label="耗材名称" :label-width="formLabelWidth" prop="officeMaterialName">
          <el-input v-model="form.officeMaterialName" style="width: 90%;"></el-input>
        </el-form-item>
        <el-form-item label="公司编号" :label-width="formLabelWidth" prop="companyId">
          <el-input v-model="form.companyId" style="width: 90%;"></el-input>
        </el-form-item>
        <el-form-item label="价格" :label-width="formLabelWidth" prop="officeMaterialPurchasePrice">
          <el-input v-model="form.officeMaterialPurchasePrice" style="width: 90%;"></el-input>
        </el-form-item>
        <el-form-item label="时间" :label-width="formLabelWidth" prop="officeMaterialPurchaseDate">
          <el-date-picker type="date" placeholder="选择日期" v-model="form.officeMaterialPurchaseDate" style="width: 90%;"
            format="yyyy 年 MM 月 dd 日" value-format="yyyy-MM-dd HH:mm:ss"></el-date-picker>
        </el-form-item>
        <el-form-item label="交易状态" :label-width="formLabelWidth" prop="payState">
          <el-input v-model="form.payState" style="width: 90%;"></el-input>
        </el-form-item>
      </el-form>
...
<script>
  ...
  } from '@/api/officeMaterialPurchase';
  export default {
    data() {
      return {
        ...
        officeMaterialPurchaseList: [], //用于存放doc数据
        showButton2: false,
        selectData: "officeMaterialPurchaseId", //被选择的下拉
        inputData: "",
        queryData: "", //用于条件查询
        dialogFormVisible: false,
        propertyList: [{
          value: 'officeMaterialPurchaseId',
          label: '编号'
        }, {
          value: 'officeMaterialName',
          label: '耗材名称'
        }, {
          value: 'companyId',
          label: '公司编号'
        ...
        form: {
          officeMaterialName: '',
          companyId: '',
          officeMaterialPurchasePrice: '',
          officeMaterialPurchaseDate: '',
          payState: ''
        },
    ...
    methods: {
      ...
      initList() {
        ...
        getPage(data).then((res) => {
          ...
          this.officeMaterialPurchaseList = res.datas;
      ...
      handleDelete(index, row) {
        console.log(row.officeMaterialPurchaseId);
        let data = {
          id: row.officeMaterialPurchaseId
        }
        deleteById(data).then((res) => {
          ...
          if (res.code=20000) {
            this.$notify({
              ...
              message: h('i', {
                style: 'color: teal'
              }, '编号为' + row.officeMaterialPurchaseId + '的记录已被删除')
            });
            this.initList();
          } else {
            this.$notify({
              title: '删除编号为' + row.officeMaterialPurchaseId + '的记录失败！',
              message: h('i', {
                style: 'color: teal'
              }, '')
            });
      ...
      handleEdit(index, row) {
        this.editId = row.officeMaterialPurchaseId;
        this.form.officeMaterialName = row.officeMaterialName;
        this.form.companyId = row.companyId;
        this.form.officeMaterialPurchasePrice = row.officeMaterialPurchasePrice;
        this.form.officeMaterialPurchaseDate = row.officeMaterialPurchaseDate;
        this.form.payState = row.payState;
      },
      handleAdd() {
        this.editId = -1;
        this.form.officeMaterialName = '';
        this.form.companyId = '';
        this.form.officeMaterialPurchasePrice = '';
        this.form.officeMaterialPurchaseDate = '';
        this.form.payState = '';
      },
      submitForm(formName) {
        this.$refs[formName].validate((valid) => {
          if (valid) {
            if (this.editId == -1) {
              let data = {
                officeMaterialName: this.form.officeMaterialName,
                companyId: this.form.companyId,
                officeMaterialPurchasePrice: this.form.officeMaterialPurchasePrice,
                officeMaterialPurchaseDate: this.form.officeMaterialPurchaseDate,
              ...
              let data = {
                ...
                officeMaterialPurchaseDate: this.form.officeMaterialPurchaseDate,
                payState: this.form.payState,
                officeMaterialPurchaseId: this.editId
              }
              edit(data).then((res) => {
                const h = this.$createElement;
                this.$notify({
