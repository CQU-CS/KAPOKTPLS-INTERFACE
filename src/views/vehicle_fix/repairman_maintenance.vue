  ...
  <div>
      ...
      <el-card class="box-card" shadow="always" :body-style="{padding: '0px'}">
        <div style="margin-left: 15px;margin-right: 15px;">
          <el-table v-loading="loading" element-loading-text="拼命加载中"
            element-loading-background="rgba(255, 255, 255, 0.5)" :height="fullHeight" :data="repairRecordList" stripe
            style="width: 100%;" :row-style="{height:'40px'}" :cell-style="{padding:'0px'}">
            <el-table-column width="80px;" align="center" prop="repairRecordId" label="编号" sortable>
            </el-table-column>
            <el-table-column align="center" show-overflow-tooltip prop="personName" label="维修人">
            </el-table-column>
            <el-table-column align="center" width="200px;" prop="truckRepairRecordId" label="车辆维修编号">
            </el-table-column>
    ...
    <el-dialog :title="dialogName" :visible.sync="dialogFormVisible" center width="40%">
      <el-form :model="form" :rules="rules" ref="form" style="text-align: center;">
        <el-form-item label="维修人编号" :label-width="formLabelWidth" prop="personId">
          <el-input v-model="form.personId" style="width: 90%;"></el-input>
        </el-form-item>
        <el-form-item label="车辆维修编号" :label-width="formLabelWidth" prop="truckRepairRecordId">
          <el-input v-model="form.truckRepairRecordId" style="width: 90%;"></el-input>
        </el-form-item>
      </el-form>
  ...
  export default {
    data() {
      return {
        ...
        repairRecordList: [], //用于存放doc数据
        showButton2: false,
        selectData: "repairRecordId", //被选择的下拉
        inputData: "",
        queryData: "", //用于条件查询
    ...
    methods: {
      ...
      initList() {
        ...
        getPage(data).then((res) => {
          ...
          this.repairRecordList = res.datas;
      ...
      handleDelete(index, row) {
        console.log(row.repairRecordId);
        let data = {
          id: row.repairRecordId
        }
        deleteById(data).then((res) => {
          ...
          if (res.code=20000) {
            this.$notify({
              ...
              message: h('i', {
                style: 'color: teal'
              }, '编号为' + row.repairRecordId + '的记录已被删除')
            });
            this.initList();
          } else {
            this.$notify({
              title: '删除编号为' + row.repairRecordId + '的记录失败！',
              message: h('i', {
                style: 'color: teal'
              }, '')
            });
      ...
      handleEdit(index, row) {
        this.editId = row.repairRecordId;
        this.form.personId = row.personId;
        this.form.truckRepairRecordId = row.truckRepairRecordId;
      },
      handleAdd() {
        this.editId = -1;
        this.form.personId = '';
        this.form.truckRepairRecordId = '';
      },
      submitForm(formName) {
          ...
          if (valid) {
            if (this.editId == -1) {
              let data = {
                ...
                truckRepairRecordId: this.form.truckRepairRecordId,
              }
              add(data).then((res) => {
                const h = this.$createElement;
                this.$notify({
