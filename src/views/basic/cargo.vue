      ...
      <el-card class="box-card" shadow="always" :body-style="{padding: '0px'}">
        <div style="margin-left: 15px;margin-right: 15px;">
          <el-table v-loading="loading" element-loading-text="拼命加载中"
            element-loading-background="rgba(255, 255, 255, 0.5)" :height="fullHeight" :data="goodsList" stripe
            style="width: 100%;" :row-style="{height:'40px'}" :cell-style="{padding:'0px'}">
            <el-table-column width="80px;" align="center" prop="goodsId" label="编号" sortable>
            </el-table-column>
            <el-table-column align="center" show-overflow-tooltip prop="goodsName" label="货物名称">
            </el-table-column>
            <el-table-column align="center" prop="goodsType" label="货物类型" >
            </el-table-column>
            <el-table-column align="center" show-overflow-tooltip prop="goodsSpecification" label="参数规格">
            </el-table-column>
    ...
    methods: {
      ...
      handleDelete(index, row) {
        ...
        deleteGoods(data).then((res) => {
          ...
          if (res.code=20000) {
            this.$notify({
              title: '删除' + row.goodsName + '成功！',
              message: h('i', {
                style: 'color: teal'
              }, '编号为' + row.goodsId + '的货物已被删除')
            });
            this.initList();
          } else {
            this.$notify({
              title: '删除' + row.goodsName + '失败！',
              message: h('i', {
                style: 'color: teal'
              }, '')
            });
      ...
      handleEdit(index, row) {
        ...
        this.form.type = row.goodsType;
        this.form.unit = row.goodsUnit;
        this.form.num = row.goodsSpecification;
		this.form.price = row.goodsUnitPrice;
      },
      handleAdd() {
        this.editId = -1;
        this.form.name = '';
          ...
          if (valid) {
            if (this.editId == -1) {
              let data = {
                ...
                goodsSpecification: this.form.num,
                goodsUnit: this.form.unit,
				goodsUnitPrice:this.form.price
              }
