  ...
  <div>
      ...
      <el-card class="box-card" shadow="always" :body-style="{padding: '0px'}">
        <div style="margin-left: 15px;margin-right: 15px;">
          <el-table v-loading="loading" element-loading-text="拼命加载中"
            element-loading-background="rgba(255, 255, 255, 0.5)" :height="fullHeight" :data="certificateList" stripe
            style="width: 100%;" :row-style="{height:'40px'}" :cell-style="{padding:'0px'}">
            <el-table-column  align="center" prop="certificateId" label="编号" sortable width="100px">
            </el-table-column>
            <el-table-column align="center" show-overflow-tooltip prop="personName" label="人员姓名">
            </el-table-column>
            <el-table-column align="center" prop="certificateType" label="证件类型" sortable>
            </el-table-column>
            <el-table-column align="center" prop="certificateHandlingUnit" label="办理单位">
            </el-table-column>
            <el-table-column align="center" prop="certificateHandlingTime" label="办理时间">
            </el-table-column>
    ...
    <el-dialog :title="dialogName" :visible.sync="dialogFormVisible" center width="40%">
      <el-form :model="form" :rules="rules" ref="form" style="text-align: center;">
        ...
        <el-form-item label="证件类型" :label-width="formLabelWidth" prop="tel">
          <el-input v-model="form.type" style="width: 90%;"></el-input>
        </el-form-item>
        <el-form-item label="办理单位" :label-width="formLabelWidth" prop="ins">
          <el-input v-model="form.unit" style="width: 90%;"></el-input>
        </el-form-item>
        <el-form-item label="办理时间" :label-width="formLabelWidth" prop="date">
  ...
  export default {
    data() {
      return {
        ...
        certificateList: [], //用于存放doc数据
        showButton2: false,
        selectData: "certificateId", //被选择的下拉
        inputData: "",
        queryData: "", //用于条件查询
    ...
    methods: {
      ...
      initList() {
        ...
        getCertificate(data).then((res) => {
          ...
          this.certificateList = res.datas;
      ...
      handleDelete(index, row) {
        ...
        deleteCertificate(data).then((res) => {
          const h = this.$createElement;
          if (res.code=20000) {
            this.$notify({
              title: '删除' + row.personName + '证件信息成功！',
              message: h('i', {
                style: 'color: teal'
              }, '编号为' + row.certificateId + '的证件已被删除')
            });
            this.initList();
          } else {
            this.$notify({
              title: '删除' + row.personName + '证件信息失败！',
              message: h('i', {
                style: 'color: teal'
              }, '')
            });
      ...
      handleEdit(index, row) {
        this.editId = row.certificateId;
        this.form.name = row.personName;
        this.form.type = row.certificateType;
        this.form.unit = row.certificateHandlingUnit;
        this.form.date = row.certificateHandlingTime;
      },
      handleAdd() {
        this.editId = -1;
        this.form.name = '';
          ...
          if (valid) {
            if (this.editId == -1) {
              ...
              let data = {
                ...
                certificateHandlingTime: this.form.date,
                certificateHandlingUnit:this.form.unit
              }
