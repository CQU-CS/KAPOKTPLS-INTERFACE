      ...
      <el-card class="box-card" shadow="always" :body-style="{padding: '0px'}">
        <div style="margin-left: 15px;margin-right: 15px;">
          <el-table v-loading="loading" element-loading-text="拼命加载中"
            element-loading-background="rgba(255, 255, 255, 0.5)" :height="fullHeight" :data="ecdList" stripe
  ...
  export default {
    ...
    methods: {
      ...
      initList() {
        ...
        getEcd(data).then((res) => {
          res.datas.forEach((item, index) => {
            // item.test = "测试属性添加";
            // console.log(item)
          })
          // console.log(res.datas);
          this.ecdList = res.datas;
      ...
      handleDelete(index, row) {
        ...
        deleteEcd(data).then((res) => {
          const h = this.$createElement;
          if (res.code=20000) {
            this.$notify({
              title: '删除' + row.ecdName + '成功！',
              message: h('i', {
                style: 'color: teal'
              }, '编号为' + row.ecdId + '的机电设备已被删除')
            });
            this.initList();
          } else {
            this.$notify({
              title: '删除' + row.ecdName + '失败！',
              message: h('i', {
                style: 'color: teal'
              }, '')
            });
      ...
      handleEdit(index, row) {
        this.editId = row.ecdId;
        this.form.name = row.ecdName;
        this.form.bname = row.buildingName;
        this.form.unit = row.ecdSpecification;
        this.form.date = row.ecdDate;
      },
      handleAdd() {
        this.editId = -1;
        this.form.name = '';
      ...
      submitForm(formName) {
        this.$refs[formName].validate((valid) => {
          if (valid) {
            if (this.editId == -1) {
              let data = {
                ecdName: this.form.name,
                buildingName: this.form.bname,
                ecdSpecification: this.form.unit,
                ecdDate: this.form.date
              }
              addEcd(data).then((res) => {
                const h = this.$createElement;
                this.$notify({
                  title: '添加成功！',
                  message: h('i', {
                    style: 'color: teal'
                  }, '名称为' + this.form.name + '的机电设备信息已添加')
                });
              ...
              editEcd(data).then((res) => {
                const h = this.$createElement;
                this.$notify({
                  title: '编辑完成！',
                  message: h('i', {
                    style: 'color: teal'
                  }, '名称为' + this.form.name + '的机电设备信息编辑完成')
                });
                this.dialogFormVisible = false;
                this.initList();
              });
