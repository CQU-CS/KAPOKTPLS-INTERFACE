  ...
  <div>
      ...
      <el-card class="box-card" shadow="always" :body-style="{padding: '0px'}">
        <div style="margin-left: 15px;margin-right: 15px;">
          <el-table v-loading="loading" element-loading-text="拼命加载中"
            element-loading-background="rgba(255, 255, 255, 0.5)" :height="fullHeight" :data="carrierManageList" stripe
            style="width: 100%;" :row-style="{height:'40px'}" :cell-style="{padding:'0px'}">
            <el-table-column width="80px;" align="center" prop="carrierManageId" label="编号" sortable>
            </el-table-column>
			...
			<el-table-column align="center" prop="carrierManagePrice" label="价格">
            </el-table-column>
            <el-table-column align="center" prop="taskTime" label="完成日期">
            </el-table-column>
            <el-table-column width="160px;" align="right">
              ...
              <template slot-scope="scope">
                ...
                <el-popconfirm title="确定删除改承运商业务信息吗？" style="margin-left: 8px;"
    ...
    <el-dialog :title="dialogName" :visible.sync="dialogFormVisible" center width="40%">
      <el-form :model="form" :rules="rules" ref="form" style="text-align: center;">
        <el-form-item label="承运商名称" :label-width="formLabelWidth" prop="name">
          <el-input v-model="form.name" style="width: 90%;"></el-input>
        </el-form-item>
        <el-form-item label="公司名称" :label-width="formLabelWidth" prop="tel">
          <el-input v-model="form.cname" style="width: 90%;"></el-input>
        </el-form-item>
        <el-form-item label="货物名称" :label-width="formLabelWidth" prop="ins">
          <el-input v-model="form.gname" style="width: 90%;"></el-input>
        </el-form-item>
		<el-form-item label="价格" :label-width="formLabelWidth" prop="ins">
  ...
  export default {
    ...
    methods: {
      ...
      initList() {
        ...
        getCarrierManage(data).then((res) => {
          res.datas.forEach((item, index) => {
            // item.test = "测试属性添加";
            // console.log(item)
          })
          // console.log(res.datas);
          this.carrierManageList = res.datas;
        ...
        deleteCarrierManage(data).then((res) => {
          ...
          if (res.code=20000) {
            this.$notify({
              ...
              message: h('i', {
                style: 'color: teal'
              }, '编号为' + row.carrierManageId + '的承运商业务已删除')
            });
            this.initList();
          } else {
            this.$notify({
              title: '删除' + row.carrierName + '失败！',
              message: h('i', {
                style: 'color: teal'
              }, '')
            });
      ...
      handleEdit(index, row) {
        this.editId = row.carrierManageId;
