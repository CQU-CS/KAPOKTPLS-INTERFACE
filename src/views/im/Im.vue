    ...
    send() {
      ...
      if (!this.text) {
        ...
        if (typeof (WebSocket) === 'undefined') {
          ...
          const message = { from: this.user.nickname, to: this.chatUser, text: this.text }
    ...
    init() {
      ...
      if (typeof (WebSocket) === 'undefined') {
        ...
        socket.onmessage = function(msg) {
          ...
          const data = JSON.parse(msg.data) // 对收到的json数据进行解析， 类似这样的： {"users": [{"nickname": "zhang"},{ "nickname": "admin"}]}
          if (data.users) { // 获取在线人员信息
            _this.users = data.users.filter(user => user.nickname !== nickname) // 获取当前连接的所有用户信息，并且排除自身，自己不会出现在自己的聊天列表里
          } else {
